...
void *ularGo() {
	struct timespec times, times2;
	
	times.tv_sec  = 0;  
    times.tv_nsec = 75 * 1000000L; //dalam nano
	...
	if(kodok.y == (max_y - 5)){
		...
		while (!isStop) {
			...
			if((ular.y != kodok.y) || (ular.x < 5)){
				return NULL;
			}
			//cek tabrakan
			if ((ular.x == kodok.x) && (ular.y == kodok.y)) {
				printNotif("Kodok terserang ular (x_x) Main lagi? (y/t)");
				isPause = true;
			}
			
			pthread_mutex_unlock(&lock);
			nanosleep(&times , &times2);
		}
	}
}
...
int main() {
  ...
  pthread_create(&tUlar, NULL, ularGo, NULL); 
